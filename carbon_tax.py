...
def simple_utility_function(x_t, tau, x):
    return (x_t + (x)*(1/(tau*tau) - 1)) ** (1/2) -(x_t)**(1/2)

def p(r):
    return 1/r

def utility_function2(ranking, x_t_list, tau_list, x): 
    sum2 = 0
    i = 0
    for xi in x:
...
def n0(ranking, x_t_list, tau_list):
    sum2 = 0
    i = 0
    for xi in x_t_list:
...
agent_europe = agent("M. Europe", u1, 0.6, 10000) 
agent_us = agent("Mme. US", u2, 0.8, 10000)
